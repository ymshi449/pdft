...
print(title)
for separation_cutoff in range(1,5):
    ...
    nuclear_nad = []
    vp_svwn = []
    for bondlength in np.linspace(4, 5, 5):
        print("============%f==============" % bondlength)
    ...
    fig1.savefig("C6 fitting" + str(separation_cutoff))

    fig2, ax2 = plt.subplots(1, 1, figsize=(16, 12), dpi=160)
    ax2.scatter(x_HeHeccpvdz, y_HeHeccpvdz + f1_energy + f2_energy + ep + nuclear_nad)
    fig2.savefig("Energy" + str(separation_cutoff))
