...
title = "For this one I used hard radius instead of relative radius."
print(title)
for separation_cutoff in np.linspace(3, 7, 10):
    ...
    nuclear_nad = []
    pbe_energy = []
    for bondlength in np.linspace(4, 6, 10):
        print("============%f==============" % bondlength)
        ...
        energy, vp_all96, vp_fock_all96 = pdfter.find_vp_all96(100, 1000, rtol=1e-2, separation_cutoff=separation_cutoff)
        # From Hartree to Ry
        energy = energy * 2.0
        pbeE = psi4.energy("pbe/aug-cc-pvdz", molecule=Full_Molec)
        # Append info
        vp.append(vp_all96)
        ...
        nuclear_nad.append((mol.Enuc - f1.Enuc - f2.Enuc)*2)
        pbe_energy.append(pbeE*2)
    ...
    pickle.dump(data, open( "save" + str(separation_cutoff) + '.p', "wb" ))
    ...
    fig1.savefig("C6 fitting" + str(int(separation_cutoff*100)))

    fig2, ax2 = plt.subplots(1, 1, figsize=(16, 12), dpi=160)
    ax2.scatter(x_HeHeccpvdz, (y_HeHeccpvdz + pbe_energy + 5.774087411731994*2)*0.5*27.2114)
    fig2.savefig("Energy" + str(int(separation_cutoff*100)))
